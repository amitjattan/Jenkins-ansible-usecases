    ...
    stages {
        stage('Build') {
            steps {
                ...
                sh 'sudo ansible-galaxy install -r artifactory-playbook/requirements.yml'
                sh 'sudo ansible-playbook -i artifactory-playbook/inventory artifactory-playbook/playbook.yml'
            }
        }
        stage('Test') {
            steps {
